...
def get_works(class_id, userdata):
    try:
        ...
        works_data = (
            works.find({"class_id": ObjectId(class_id)})
            .sort({"time": -1})
            .limit(10)
            .skip(skip)
        )
        return jsonify(
            {"success": True, "works": json.loads(json_util.dumps(works_data))}
        )
...
def submit_work(class_id, work_id, userdata):
    try:
        ...
        count = submits.count_documents(
            {"work_id": ObjectId(work_id), "user_id": ObjectId(userdata["userid"])}
        )
        ...
        if not work["accept_submits"]:
            return jsonify(
                {
                    "success": False,
                    "message": "the work is not accepting submissions now!",
                }
            )
                ...
                if "answer" in question and str(question_index) in data:
                    if question["type"] == "MCQ":
                        ...
                        channel.basic_publish(
                            exchange="",
                            routing_key="Descriptive_Q",
                            body=json.dumps(
                                {
                                    "correct": question["answer"],
                                    "student": data[str(question_index)],
        ...
        submits.insert_one(
            {
                ...
                "user_id": ObjectId(userdata["userid"]),
                "response": data,
                "marks": marks,
                "mark": mark,
    ...
    try:
        ...
        if userdata["role"] == "teacher":
            ...
            return jsonify(
                {
                    "success": True,
                    "class": json.loads(json_util.dumps(class_)),
                    "work": json.loads(json_util.dumps(work)),
                }
            )
    except Exception as e:
        return (
...
def get_submits(class_id, work_id, userdata):
    try:
        submissions = submits.aggregate(
            [
                {"$match": {"work_id": ObjectId(work_id)}},
                {
                    "$lookup": {
                        "from": "users",
        ...
        print(submissions)
        class_ = classes.find_one(
            {"_id": ObjectId(class_id)}, {"number_of_students": 1}
        )
        if not submissions:
        ...
        return jsonify(
            {
                "success": True,
                "submits": json.loads(json_util.dumps(submissions)),
                "total_students": class_["number_of_students"],
            }
        )
    except Exception as e:
        return jsonify(
...
def change_accept_submits(class_id, work_id, userdata):
    try:
        ...
        works.update_one(
            ...
            {"$set": {"accept_submits": data["accept_submits"]}},
        )
        return jsonify(
            {
                "success": True,
                "message": (
                    "submits is accepting now!"
                    if data["accept_submits"]
                    else "submits is not accepting now!"
                ),
            }
        )
    except Exception as e:
        print(e)
        return jsonify(
            {"success": True, "message": "something went wrong", "error": str(e)}
        )
...
def export_submits(class_id, userdata):
    pipeline = [
        ...
        {"$replaceRoot": {"newRoot": {"$mergeObjects": ["$$ROOT", "$marks"]}}},
        {"$project": {"marks": 0}},
    ]
