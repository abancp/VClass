...
"""
        ...
        print(response.text)
        if "<general>" in response.text:
            if response.text.startswith("```"):
                response.text.removeprefix("```").removesuffix("```").split(" ")[1]
            return jsonify({"success":True,"response":response.text.replace("<general>","")})
        query = response.text
