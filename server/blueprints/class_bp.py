...
def create_class(userdata):
    try:
        ...
        data["settings"] = {
            "announcements": {
                "post_permission": {"teacher": True, "student": True},
            },
            "doubts": {"comment_permission": "everyone", "ai_comment": "all"},
        }

        inserted_class = classes.insert_one(data)
        users.update_one(
            {"_id": ObjectId(userdata["userid"])},
            {"$push": {"teacher": inserted_class.inserted_id}},
        )
        ...
        res = make_response(
            jsonify(
                {
                    "success": True,
                    "classid": str(inserted_class.inserted_id),
                    "message": "created class : " + data["name"],
                }
            )
        )
            ...
            return jsonify(
                {
                    "success": True,
                    "role": "student",
                    "class": json_util.loads(json_util.dumps(class_)),
                }
            )
        return jsonify(
            {
...
def get_peoples(class_id, userdata):
    ...
    return jsonify(
        {"peoples": json_util.loads(json_util.dumps(peoples))[0], "success": True}
    )
...
def join_class(userdata):
    try:
        ...
        if (
            found_class["_id"] in user["teacher"]
            or found_class["_id"] in user["student"]
        ):
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "users already joined",
                        "classid": str(found_class["_id"]),
                    }
                ),
                409,
            )
        users.update_one(
            {"_id": userObjectId}, {"$push": {"student": found_class["_id"]}}
        )
        classes.update_one(
            {"_id": found_class["_id"]},
            {
                "$push": {"students": ObjectId(userdata["userid"])},
                "$inc": {"number_of_students": 1},
            },
        )
        print(userdata)
        token = jwt.encode(userdata, os.getenv("JWT_SECRET"), algorithm="HS256")
...
def join_public_class(userdata):
    try:
        ...
        redis_client.delete(
            f"user_roles:{userdata['userid']}_{
                str(data['_id'])}"
        )
        res = make_response(
            jsonify(
...
def get_classes(userdata):
    pipeline = [
        ...
        {"$set": {"combined_roles": {"$concatArrays": ["$student", "$teacher"]}}},
        {
            "$lookup": {
                "from": "classes",
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "classes": json_util.loads(json_util.dumps(list(class_details))),
            }
        )
    except Exception as e:
        print(e)
...
def remove_student(userdata):
    ...
    users.update_one(
        ...
        {"$pull": {"student": ObjectId(data["class_id"])}},
    )
    redis_client.delete(f"user_roles:{data['user_id']}_{data['class_id']}")
...
def make_teacher(userdata):
    try:
        ...
        classes.update_one(
            ...
            {
                "$pull": {"students": ObjectId(data["user_id"])},
                "$push": {"teachers": ObjectId(data["user_id"])},
            },
        )
        users.update_one(
            {"_id": ObjectId(data["user_id"])},
            {
                "$pull": {"student": ObjectId(data["class_id"])},
                "$push": {"teacher": ObjectId(data["class_id"])},
            },
        )
        redis_client.delete(f"user_roles:{data['user_id']}_{data['class_id']}")
...
def remove_teacher(userdata):
    try:
        ...
        if userdata["userid"] == data["user_id"]:
            return jsonify(
                {"success": False, "message": "Yourself can't remove from teacher"}
            )
        classes.update_one(
            {"_id": ObjectId(data["class_id"])},
            {
                "$push": {"students": ObjectId(data["user_id"])},
                "$pull": {"teachers": ObjectId(data["user_id"])},
            },
        )
        users.update_one(
            {"_id": ObjectId(data["user_id"])},
            {
                "$push": {"student": ObjectId(data["class_id"])},
                "$pull": {"teacher": ObjectId(data["class_id"])},
            },
        )
        redis_client.delete(f"user_roles:{data['user_id']}_{data['class_id']}")
        return jsonify({"success": True, "message": "Teacher added"})
...
def create_info(class_id, userdata):
    try:
        data = request.get_json()
        db["infos"].replace_one(
            {"class_id": ObjectId(class_id)},
            {"class_id": ObjectId(class_id), "info": data["info"]},
            upsert=True,
        )
...
def export_submits(class_id, userdata):
    ...
    data = list(db["atts"].find({"class_id": ObjectId(class_id)}))
    print(data)
