    ...
    <header className='backdrop-blur-sm  bg-center bg-no-repeat h-[3.5rem] w-full  top-0 left-0 fixed z-[100] text-light  px-4 py-2 justify-between items-center gap bg-secondery/40 gap-3 flex'>
      ...
      {forWhat === "home" &&
        <div className='flex gap-2 justify-center text-[17px] font-semibold w-[33.3%]'>
          ...
          <div onClick={() => setTab("joined")} className={`p-1 hover:bg-secondery ${tab === "joined" && "bg-tersiory hover:bg-tersiory"} rounded-md px-2 cursor-pointer `}>Joined</div>
          <div onClick={() => setTab("public")} className={`p-1 hover:bg-secondery ${tab === "public" && "bg-tersiory hover:bg-tersiory"} rounded-md px-2 cursor-pointer `}>Public</div>
        </div>
      }
      <div className="flex gap-3 w-[33.4%] justify-end">
